...
const DEFAULT_REPO_NAME = 'threatcrush-apt';
...
export class APTPackageManager extends BasePackageManager {
  ...
  async checkExisting(version: string): Promise<boolean> {
    try {
      ...
      const path = `pool/main/p/threatcrush/threatcrush_${version}_amd64.deb`;
...
Pool structure:
  pool/main/p/threatcrush/threatcrush_${release.version}_amd64.deb
...
`);
  ...
  async submit(release: ReleaseInfo, dryRun = false): Promise<SubmissionResult> {
    ...
    try {
      ...
      // Ensure repo exists
      await this.ensureRepo(this.repoOwner, this.repoName, 'APT repository for ThreatCrush', false);
      ...
      const poolDir = join(repoDir, 'pool/main/p/threatcrush');
      ...
      const debFilename = `threatcrush_${release.version}_amd64.deb`;
      ...
      if (gpgPublicKey) {
        writeFileSync(join(repoDir, 'threatcrush.gpg'), gpgPublicKey);
      }

      // Create README with installation instructions
      const readmeContent = `# ThreatCrush APT Repository

## Installation

\`\`\`bash
# Add GPG key
curl -fsSL https://${this.repoOwner}.github.io/${this.repoName}/threatcrush.gpg | sudo gpg --dearmor -o /usr/share/keyrings/threatcrush.gpg

# Add repository
echo "deb [signed-by=/usr/share/keyrings/threatcrush.gpg] https://${this.repoOwner}.github.io/${this.repoName} stable main" | sudo tee /etc/apt/sources.list.d/threatcrush.list

# Install
sudo apt update
sudo apt install threatcrush
...
`;
  ...
  private generatePackagesFile(release: ReleaseInfo, size: number): string {
    return `Package: threatcrush
Version: ${release.version}
Architecture: amd64
Maintainer: ThreatCrush Team <hello@threatcrush.com>
...
Filename: pool/main/p/threatcrush/threatcrush_${release.version}_amd64.deb
Size: ${String(size)}
Section: net
Priority: optional
Homepage: https://threatcrush.com
Description: Collaborative screen sharing with remote control
 ThreatCrush enables real-time screen sharing with simultaneous local
...
`;
  ...
  private generateReleaseFile(repoDir: string, distsDir: string): string {
    ...
    return `Origin: ThreatCrush
Label: ThreatCrush
Suite: stable
Codename: stable
Architectures: amd64
Components: main
Description: ThreatCrush APT Repository
Date: ${date}
SHA256:
