...
export class AURPackageManager extends BasePackageManager {
  ...
  generateManifest(release: ReleaseInfo): Promise<string> {
    ...
    const pkgbuild = `# Maintainer: ThreatCrush Team <hello@threatcrush.com>
...
provides=('threatcrush')
conflicts=('threatcrush' 'threatcrush-git')
options=('!strip')
source=("ThreatCrush-\${pkgver}.AppImage::https://github.com/profullstack/threatcrush/releases/download/v\${pkgver}/ThreatCrush-\${pkgver}-x86_64.AppImage")
sha256sums=('${sha256}')
...
package() {
    ...
    # Install AppImage
    install -Dm755 "ThreatCrush-\${pkgver}.AppImage" "$pkgdir/opt/threatcrush/threatcrush.AppImage"

    # Create wrapper script
    install -dm755 "$pkgdir/usr/bin"
    cat > "$pkgdir/usr/bin/threatcrush" << 'WRAPPER'
#!/bin/bash
export ELECTRON_DISABLE_SANDBOX=1
exec /opt/threatcrush/threatcrush.AppImage "$@"
WRAPPER
    chmod 755 "$pkgdir/usr/bin/threatcrush"

    # Create and install desktop file
    cat > "$srcdir/threatcrush.desktop" << 'DESKTOP'
[Desktop Entry]
Name=ThreatCrush
Comment=Collaborative screen sharing with remote control
Exec=/opt/threatcrush/threatcrush.AppImage --no-sandbox %U
Icon=threatcrush
Type=Application
Categories=Network;RemoteAccess;
StartupWMClass=ThreatCrush
DESKTOP
    install -Dm644 "$srcdir/threatcrush.desktop" "$pkgdir/usr/share/applications/threatcrush.desktop"

    # Extract and install icon from AppImage
    cd "$pkgdir/opt/threatcrush"
    ./threatcrush.AppImage --appimage-extract usr/share/icons/hicolor/512x512/apps/*.png 2>/dev/null || true
    if [ -f squashfs-root/usr/share/icons/hicolor/512x512/apps/*.png ]; then
        install -Dm644 squashfs-root/usr/share/icons/hicolor/512x512/apps/*.png "$pkgdir/usr/share/pixmaps/threatcrush.png"
    fi
    rm -rf squashfs-root
}
`;
...
\tprovides = threatcrush
\tconflicts = threatcrush
\tconflicts = threatcrush-git
\toptions = !strip
\tsource = ThreatCrush-${version}.AppImage::https://github.com/profullstack/threatcrush/releases/download/v${version}/ThreatCrush-${version}-x86_64.AppImage
...
`;
      ...
      // Configure git
      execSync('git config user.email "hello@threatcrush.com"', { cwd: repoDir, env, stdio: 'pipe' });
      execSync('git config user.name "ThreatCrush Bot"', { cwd: repoDir, env, stdio: 'pipe' });
