...
export class ChocolateyPackageManager extends BasePackageManager {
  ...
  generateManifest(release: ReleaseInfo): Promise<Record<string, string>> {
    ...
    // Note: checksums file may have spaces in filename (e.g., "ThreatCrush Setup 0.1.17.exe")
    // while GitHub asset has dots (e.g., "ThreatCrush.Setup.0.1.17.exe")
    const checksum =
      exe?.sha256 ??
      release.checksums.get(`ThreatCrush Setup ${release.version}.exe`) ??
      release.checksums.get(`ThreatCrush-${release.version}-x64.exe`) ??
      '';
...
<package xmlns="http://schemas.microsoft.com/packaging/2015/06/nuspec.xsd">
  <metadata>
    ...
    <title>ThreatCrush</title>
    <authors>Profullstack, Inc.</authors>
    <owners>profullstack</owners>
    <projectUrl>https://threatcrush.com</projectUrl>
    <iconUrl>https://threatcrush.com/logo.svg</iconUrl>
    <licenseUrl>https://github.com/profullstack/threatcrush/blob/master/LICENSE</licenseUrl>
    <requireLicenseAcceptance>false</requireLicenseAcceptance>
    <projectSourceUrl>https://github.com/profullstack/threatcrush</projectSourceUrl>
    <docsUrl>https://threatcrush.com/docs</docsUrl>
    <bugTrackerUrl>https://github.com/profullstack/threatcrush/issues</bugTrackerUrl>
    ...
    <description>ThreatCrush is a collaborative screen sharing application with simultaneous remote mouse and keyboard control. Like Screenhero, but open source. Perfect for pair programming, remote support, and collaboration.
...
- Open source under MIT License
    ...
    <releaseNotes>https://github.com/profullstack/threatcrush/releases/tag/v${release.version}</releaseNotes>
  </metadata>
  <files>
  ...
  async submit(release: ReleaseInfo, dryRun = false): Promise<SubmissionResult> {
    ...
    try {
      ...
      try {
        ...
        try {
          execSync('nuget pack threatcrush.nuspec', { cwd: tempDir, stdio: 'pipe' });
        } catch {
          return {
            packageManager: this.name,
            status: 'failed',
