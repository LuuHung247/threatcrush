 ...
 *
 * Automatically submits ThreatCrush releases to various package managers.
 *
 * Usage:
...
const GITHUB_REPO = 'profullstack/threatcrush';
...
function showHelp(): void {
  console.log(`
${colors.cyan}ThreatCrush Package Submission Script${colors.reset}
...
async function fetchReleaseInfo(version: string): Promise<ReleaseInfo | null> {
  ...
  const headers: Record<string, string> = {
    ...
    'User-Agent': 'ThreatCrush-Submit-Packages',
  };
  if (token) {
...
async function main(): Promise<void> {
  ...
  console.log(`
${colors.cyan}╔════════════════════════════════════════════════════════════╗
║           ThreatCrush Package Submission Script                 ║
╚════════════════════════════════════════════════════════════╝${colors.reset}
`);
  ...
  for (const asset of releaseInfo.assets) {
    ...
    // If not found, try replacing "ThreatCrush.Setup" with "ThreatCrush Setup"
    if (!checksum && asset.name.startsWith('ThreatCrush.Setup.')) {
      const nameWithSpaces = asset.name.replace('ThreatCrush.Setup.', 'ThreatCrush Setup ');
